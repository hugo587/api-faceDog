...
api.login = async (req,res)=>{
    ...
    const user = await new UserDao(req.db).findByNameAndPassword(userName,password);
        ...
        if(user){
            ...
            console.log(`Authentication Tokken added to response`);
            ...
            console.log(`Authencation faled for user ${userName}`);
            console.log('No token generated');
            res.status(401).json({message:`Authentication faled for user ${userName} `});
        }
};
